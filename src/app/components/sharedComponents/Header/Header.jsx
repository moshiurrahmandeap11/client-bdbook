...
const Header = () => {
      ...
      if (data.success) {
        if (append) {
          setNotifications(prev => {
            ...
            return newNotifications.filter((n, index, self) => 
        ...
        >
          <div className="flex items-center gap-3">
            {notification.data?.senderProfilePicture ? (
              <img src={notification.data.senderProfilePicture} alt="" className="w-10 h-10 rounded-full object-cover"/>
            ) : (
            ...
            <div className="flex-1">
              <p className="text-white text-sm font-medium">{notification.data?.message || "New notification"}</p>
  ...
  }, [socket, isAuthenticated, user?._id, router, fetchUnreadMessagesCountForced, handleNotificationClick]);
  ...
  }, [isAuthenticated, initialFetchDone, fetchNotifications, fetchUnreadMessagesCountForced, fetchUnreadNotificationsCount]);
  ...
  useEffect(() => {
    ...
    return () => {
      if (messageCountInterval.current) clearInterval(messageCountInterval.current);
    };
  }, [isAuthenticated, fetchUnreadMessagesCount, fetchUnreadNotificationsCount]);
  ...
  }, [isAuthenticated, fetchUnreadMessagesCountForced, fetchUnreadNotificationsCount]);
                  ...
                  <div className="relative" ref={notificationsRef}>
                    ...
                    {isNotificationsOpen && (
                      <>
                        ...
                        />
                        <div className="absolute right-0 mt-2 w-[calc(100vw-2rem)] sm:w-96 md:w-80 lg:w-96 rounded-xl backdrop-blur-xl bg-black/90 border border-white/20 shadow-2xl overflow-hidden animate-fadeInDown z-50">
                            ...
                            {notifications.length === 0 ? (
                              ...
                              <>
                                {notifications.map((notification) => (
                                  ...
                                  >
                                    <div className={`w-10 h-10 rounded-full ${getNotificationBg(notification.type)} flex items-center justify-center text-xl flex-shrink-0`}>
