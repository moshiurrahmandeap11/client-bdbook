...
// ─── Likes Modal ───────────────────────────────────────────────────────────────
const LikesModal = ({ videoId, onClose }) => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        if (res.data.success) setLikes(res.data.data || []);
      } catch {
        // silent
      } finally {
        setLoading(false);
            ...
            likes.map((person, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {person.username && <p className="text-white/50 text-xs truncate">@{person.username}</p>}
                </div>
...
const VideoPlayer = ({ video, isMuted, isActive, onDoubleTap }) => {
  ...
  return (
    <div className="absolute inset-0 w-full h-full" onMouseMove={showCtrl} onTouchStart={showCtrl}>
      <video
        ref={videoRef}
        src={video.media?.url}
      ...
      />
      {!isPlaying && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/30 cursor-pointer" onClick={handleTap}>
          <PlayIcon className="h-16 w-16 text-white drop-shadow-lg" />
        </div>
      )}
      {(showControls || !isPlaying) && (
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4 pb-2" onClick={(e) => e.stopPropagation()}>
          <div className="w-full h-1.5 bg-white/30 rounded-full cursor-pointer mb-3" onClick={handleSeek}>
            <div className="h-full bg-purple-500 rounded-full relative" style={{ width: `${progress}%` }}>
...
const VideosPage = () => {
  ...
  const [likesModal, setLikesModal] = useState({ isOpen: false, videoId: null });
  ...
  const snapToIndex = useCallback(
    (index) => {
      ...
      if (!container) return;
      const clamped = Math.max(0, Math.min(index, allVideos.length - 1));
      container.style.transition = "transform 0.28s cubic-bezier(0.25, 0.46, 0.45, 0.94)";
      container.style.transform = `translateY(-${clamped * 100}vh)`;
      setActiveIndex(clamped);
      if (clamped >= allVideos.length - 2 && hasNextPage && !isFetchingNextPage) fetchNextPage();
    },
  ...
  const handleMouseMove = (e) => {
    ...
    if (containerRef.current)
      containerRef.current.style.transform = `translateY(${-activeIndex * window.innerHeight + diff}px)`;
  };
  const handleMouseUp = (e) => {
    ...
    if (diff < -60 && activeIndex < allVideos.length - 1) snapToIndex(activeIndex + 1);
    else if (diff > 60 && activeIndex > 0) snapToIndex(activeIndex - 1);
    else snapToIndex(activeIndex);
  };

  // ── Touch (fast for Android — 50px threshold) ──
  ...
  const handleTouchEnd = (e) => {
    ...
    dragStartY.current = null;
    if (diff < -50 && activeIndex < allVideos.length - 1) snapToIndex(activeIndex + 1);
    else if (diff > 50 && activeIndex > 0) snapToIndex(activeIndex - 1);
    else snapToIndex(activeIndex);
  };

  // ── Wheel (throttled 600ms) ──
  const handleWheel = useCallback(
    (e) => {
      ...
      if (e.deltaY > 0 && activeIndex < allVideos.length - 1) snapToIndex(activeIndex + 1);
      else if (e.deltaY < 0 && activeIndex > 0) snapToIndex(activeIndex - 1);
    },
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center px-6">
          ...
          {isAuthenticated && (
            <Link href="/create-post" className="inline-block mt-4 px-6 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl hover:scale-105 transition-transform">
  ...
  return (
    <>
      ...
      <div id="video-wrapper" className="fixed inset-0 bg-black overflow-hidden touch-none select-none">
        ...
        <div className="fixed right-2 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-1.5">
          {allVideos.slice(0, 10).map((_, i) => (
            <button
              ...
              className={`w-1.5 rounded-full transition-all duration-300 ${i === activeIndex ? "h-6 bg-purple-500" : "h-1.5 bg-white/30"}`}
            />
          ))}
        </div>
            ...
            return (
              ...
              >
                ...
                {floatingHearts[video._id] && (
                  <FloatingHeart onDone={() => setFloatingHearts((p) => { const n = { ...p }; delete n[video._id]; return n; })} />
                )}
                ...
                <div className="absolute right-3 bottom-28 sm:bottom-32 flex flex-col items-center gap-5 z-10">
                  <VideoAction
                    ...
                    onLongPress={() => setLikesModal({ isOpen: true, videoId: video._id })}
                  />
                  <VideoAction
          ...
          >
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-white/10">
                ...
                <button onClick={() => setCommentModal({ isOpen: false, video: null, index: null })} className="p-1 rounded-full bg-white/10 hover:bg-white/20 transition">
                  <XMarkIcon className="h-5 w-5 text-white" />
                </button>
              </div>
              ...
              <div className="p-4 text-center border-t border-white/10">
                <button
                  onClick={() => { setCommentModal({ isOpen: false, video: null, index: null }); window.location.href = `/post/details/${commentModal.video?._id}`; }}
        ...
        {likesModal.isOpen && (
          <LikesModal
            videoId={likesModal.videoId}
            onClose={() => setLikesModal({ isOpen: false, videoId: null })}
          />
        )}
      </div>
    </>
